...
https://docs.google.com/document/d/1_t2OxTHCI5C9Qvu75GMgy1K_BiuSx8cP8z4XS_F-UiE/edit
...
print("TIC VALUE COUNTS:",df.tic.value_counts())
'''
...
for i in range(rebalance_window+validation_window, len(unique_trade_date)+1,rebalance_window):
    ...
    # print(temp.shape)
    df_account_value = pd.concat([df_account_value, temp], ignore_index=True)
    # df_account_value = df_account_value.append(list(temp),ignore_index=True)
#     pd.concat([df_account_value, pd.DataFrame([temp])], ignore_index=True)
sharpe=(252**0.5)*df_account_value.account_value.pct_change(1).mean()/df_account_value.account_value.pct_change(1).std()
print('Sharpe Ratio: ',sharpe)
df_account_value=df_account_value.join(df_trade_date[validation_window:].reset_index(drop=True))
...
trading_client = TradingClient('PK6OLMQA3TSKGUT30NG7', 'Jb2dE9ir20DlosSvfkP9YI8Tywtec9KJPP446cy6', paper=True)
